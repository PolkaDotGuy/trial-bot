...
class afk(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        Also detects when the user sends a message which is not a command
        and replies correspondingly.
        '''
        ...
        if (message.author.id in self.data.keys() and 
        not "afk" in message.content):
            if message.channel != self.data[message.author.id][1]:
                await self.data[message.author.id][1].send(
                    f"<@{message.author.id}> is back!")
